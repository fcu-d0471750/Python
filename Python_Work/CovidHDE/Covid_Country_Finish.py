...
# 對資料排序
def race_data_sort(data, input_day , mod):
    if mod == 'barh':
        # 獲得符合input_day的資料
...
def race_barchart(input_day):
    # 對資料排序
    dff = race_data_sort(done_confirmed, input_day,'barh')
    deaths = race_data_sort(done_deaths, input_day,'barh')
    recover = race_data_sort(done_recovered, input_day,'barh')

    # 清空畫布
    ax.clear()

    # 長條圖(確診)
    ax.barh(np.arange(len(dff['Country/Region'])), dff['Value'], height=0.2,
            color=[colors[x] for x in dff['Country/Region']], edgecolor='#666666', hatch='/', label="confiemed")
    # 長條圖(死亡)
    ax.barh(np.arange(len(dff['Country/Region'])) - 0.3, deaths['Value'], height=0.2,
            color=[colors[x] for x in dff['Country/Region']], edgecolor='#666666', hatch='-', label="deaths")
    # 長條圖(康復)
    ax.barh(np.arange(len(dff['Country/Region'])) - 0.6, recover['Value'], height=0.2,
            color=[colors[x] for x in dff['Country/Region']], edgecolor='#666666', hatch='+', label="recovered")
    ...
    # 增加總確診人數
    ax.text(0.82, 0.11, 'Total(con): ' + '{:,.0f}'.format(dff['Value'].sum()), transform=ax.transAxes, size=12,
            color='#777777', ha='right')
    # 增加總死亡人數
    ax.text(0.82, 0.07, 'Total(de): ' + '{:,.0f}'.format(deaths['Value'].sum()), transform=ax.transAxes, size=12,
            color='#777777', ha='right')
    # 增加總康復人數
    ax.text(0.82, 0.03, 'Total(re): ' + '{:,.0f}'.format(recover['Value'].sum()), transform=ax.transAxes, size=12,
            color='#777777', ha='right')
...
def race_plotchart(input_day):
    # 對資料排序
    dff = race_data_sort(done_confirmed, input_day,'plot')
